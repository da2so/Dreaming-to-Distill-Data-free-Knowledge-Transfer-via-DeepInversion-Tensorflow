...
def basic_block(filters, init_strides=1, is_first_block_of_first_layer=False):
    ...
    def f(input):
        ...
        out = ZeroPadding2D(padding=1)(input)
        out = Conv2D(filters=filters, kernel_size=3, strides=init_strides)(out)
        out= _bn_relu(out)

        out = Conv2D(filters=filters, kernel_size=3, strides=1,padding="same")(out)
        residual = BatchNormalization(axis=CHANNEL_AXIS)(out)
...
def bottleneck(filters, init_strides=1, is_first_block_of_first_layer=False):
    ...
    def f(input):
        ...
        out = Conv2D(filters=filters, kernel_size=1, strides=1)(input)
        out = _bn_relu(out)
        ...
        out = Conv2D(filters=filters*4, kernel_size=1, strides=1)(out)
        residual = BatchNormalization(axis=CHANNEL_AXIS)(out)
...
class ResNet(object):
    def build(input_shape, num_outputs, block_fn, repetitions):
        ...
        conv1 = Conv2D(filters=filters_num, kernel_size=3, strides=1, padding='same')(input)
        norm1 = _bn_relu(conv1)
        #pool1 = MaxPooling2D(pool_size=3, strides=2, padding="same")(norm1)
        ...
        pool2 = AveragePooling2D(pool_size=block_shape[ROW_AXIS], strides=1)(block)
