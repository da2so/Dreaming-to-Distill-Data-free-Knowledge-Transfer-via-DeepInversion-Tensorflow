...
from utils import load_model, data_info , pack_images
...
class Deep_Inversion():
    def __init__(self,dataset,t_model_path,adi_coeff,s_model_path, r_feature, tv_l1, \
        tv_l2, l2 ,lr , n_iters, first_bn_mul, main_mul ,bs, jitter, random_label, save_path, epochs=0):
        ...
        for e in range(self.epochs):
            ...
            for i_iter in range(self.n_iters+1):
                ...
                if i_iter %  100== 0:
                    ...
                    save_path_root=self.save_path+'/'+self.dataset+'/'
                    save_path_for_pack=save_path_root+'pack_imgs/'
                    save_path_for_imgs=save_path_root+'imgs/'
                    if not os.path.exists(save_path_for_pack):
                        os.makedirs(save_path_for_pack)
                    if not os.path.exists(save_path_for_imgs):
                        os.makedirs(save_path_for_imgs)
                        
                    pack_imgs=pack_images(inputs,self.bs)
                    #save multiple images
                    k.preprocessing.image.save_img(f'{save_path_for_pack}{str(i_iter // 100)}.png', pack_imgs, scale=False)
                    
                    if i_iter == self.n_iters :
                        for bs in range(self.bs):
                            #save each image for training student network
                            k.preprocessing.image.save_img(f'{save_path_for_imgs}{str((e*self.bs)+(bs+1))}.png', inputs[bs,:,:,:])
